...
| `minimize_window` | `{"id":1,"op":"minimize_window","window":5}` | Reuses `shell::ssd::minimize_to_dock` (unmap + track as minimized, restore via `Dock.app`) |
| `restore_window` | `{"id":1,"op":"restore_window","window":5}` | Restores a minimized window (re-map centered + focus, drop minimized tag) via `shell::ssd::restore_minimized`; errors when the id is not minimized or its client is gone |
